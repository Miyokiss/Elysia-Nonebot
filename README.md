...
#### 生视频
- **功能描述**: 使用 Seedance 2.0 生成视频，支持文字、图片与参考视频输入。
- **指令**: `/生视频 <提示词>`，也可在同消息附带媒体或引用含媒体消息。
...
<b>🚫注意：</b>
将你的 `app_id`、各功能所需的 API 密钥替换为实际值，然后将文件重命名为 **api_config.py**。已有本地 `api_config.py` 的部署也需要补充上述配置。GPT 生图、Seedream 生图、生图助手和生视频分别使用各自的 `base_url`、`api_key` 与 `model`；它们可以指向同一网关，也可以完全独立。上游总用量由各密钥自身的额度决定。生图与生视频端点会接收密钥、提示词和参考媒体，生产环境应使用 HTTPS；HTTP 仅适用于完全可信的内网。
...
```text
/生视频 提示词
/生视频 图生视频 提示词（同消息附图，或引用含图消息）
/生视频 参考视频 提示词（同消息附视频，或引用含视频消息）
```

不写模式时会根据附件自动判断：无媒体为文生视频，附图为图生视频，附视频为参考视频；参考视频模式也可以同时附带最多 4 张参考图。当前消息中的媒体优先于被引用消息。单张参考图上限为 10 MiB、参考图合计上限为 20 MiB，参考视频上限为 50 MiB。生成任务默认最长等待 15 分钟，并限制为单任务并发、每位用户 5 分钟冷却和每天 3 次；这些值均可在 `api_config.py` 中调整，每日限制设为 `0` 表示不限制。
