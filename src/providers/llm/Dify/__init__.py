...
class DifyChatRole(Model):
    ...
    class Meta:
        # 指定表名
        table = "dify_chat_role"
        table_description = "Dify聊天表"
    ...
    async def create_chat_role(cls, user_id: List[str], is_session_id: str, memory_id: str, like_value: int) -> Self:
        ...
        示例:
        await DifyChatRole.create_chat_role(user_id=["12345"], is_session_id="session_123", memory_id="memory_123")
        """
        try:
    ...
    async def get_chat_role_by_user_id(cls, user_id: List[str]) -> Optional[Self]:
        ...
        示例:
        chat_role = await DifyChatRole.get_chat_role_by_user_id(user_id=["12345"])
        """
        try:
    ...
    async def update_chat_role_by_user_id(cls, user_id: List[str], **kwargs) -> bool:
        ...
        示例:
        success = await DifyChatRole.update_chat_role_by_user_id(user_id=["12345"], is_session_id="new_session_id")
        """
        try:
    ...
    async def delete_chat_role_by_user_id(cls, user_id: List[str]) -> bool:
        ...
        示例:
        success = await DifyChatRole.delete_chat_role_by_user_id(user_id=["12345"])
        """
        try:
